...
function authenticate(opts) {
	...
	function sendRequest(resolve, reject, err, res) {
		let authHeader;
		...
		if (ntlmState.status === 0) {
			...
			return resolve(ntlmRequest);
		}

		ntlmState.status++;

		ntlmRequest(createRequestOptions(authHeader), sendRequest.bind(this, resolve, reject));
	}
